...
const WebshopSwitcher: React.FC<WebshopSwitcherProps> = ({ currentWebshop, onWebshopChange }) => {
  ...
  // Mock webshops data
  const mockWebshops: Webshop[] = [
    {
      id: 1,
      name: 'Fashion Store Demo',
      domain: 'fashion-demo.nl',
  ...
  useEffect(() => {
    setWebshops(mockWebshops);
    if (!currentWebshop) {
      onWebshopChange(mockWebshops[0]);
    }
  }, []);
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
          ...
          <div className="p-4 border-t border-gray-200">
            <button className="w-full flex items-center justify-center space-x-2 px-4 py-2 border border-primary-300 text-sm font-medium rounded-md text-primary-700 bg-primary-50 hover:bg-primary-100">
