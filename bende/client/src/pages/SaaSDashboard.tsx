...
const SaaSDashboard: React.FC = () => {
  ...
  const mockMetrics: SaaSMetrics = {
    mrr: 12450, // €12,450 MRR
    arr: 149400, // €149,400 ARR
    total_customers: 186, // 186 webshops using the CRM
    new_customers_month: 23,
    churned_customers: 3,
    arpu: 66.94, // Average Revenue Per User
    churn_rate: 1.6, // 1.6% monthly churn
    ltv: 4183.75 // Customer Lifetime Value
  };

  const mockPlans: SubscriptionPlan[] = [
    { id: 1, name: 'Free Trial', price_monthly: 0, price_yearly: 0, customers: 45, mrr_contribution: 0 },
    { id: 2, name: 'Starter', price_monthly: 29, price_yearly: 290, customers: 89, mrr_contribution: 2581 },
    { id: 3, name: 'Professional', price_monthly: 79, price_yearly: 790, customers: 41, mrr_contribution: 3239 },
    { id: 4, name: 'Enterprise', price_monthly: 199, price_yearly: 1990, customers: 11, mrr_contribution: 2189 }
  ];
  ...
  const revenueGrowth = Array.from({length: 12}, (_, i) => {
    ...
    return {
      month: month.toLocaleDateString('nl-NL', { month: 'short' }),
      mrr: Math.floor(8000 * growth),
      customers: Math.floor(120 * growth),
      arpu: 65 + (i * 2)
    };
  });

  const integrationStats = [
    { name: 'Shopify', users: 67, color: '#95BF47' },
    { name: 'WooCommerce', users: 45, color: '#96588A' },
    { name: 'Magento', users: 23, color: '#EC6737' },
    { name: 'Custom', users: 51, color: '#3B82F6' }
  ];
