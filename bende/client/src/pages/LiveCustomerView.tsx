...
const LiveCustomerView: React.FC = () => {
    ...
    const mockSessions: CustomerSession[] = [
      {
        ...
        activities: [
          { type: 'navigation', page: '/customer-portal', timestamp: new Date(Date.now() - 10 * 60 * 1000) },
          { type: 'click', element: 'Mijn Bestellingen tab', timestamp: new Date(Date.now() - 5 * 60 * 1000) },
          { type: 'scroll', position: { x: 0, y: 300 }, timestamp: new Date(Date.now() - 2 * 60 * 1000) }
        ],
        isOnline: true,
      ...
      },
      {
        ...
        activities: [
          { type: 'navigation', page: '/customer-login', timestamp: new Date(Date.now() - 8 * 60 * 1000) },
          { type: 'form_fill', element: 'Login form', timestamp: new Date(Date.now() - 7 * 60 * 1000) },
          { type: 'click', element: 'Profiel tab', timestamp: new Date(Date.now() - 3 * 60 * 1000) }
        ],
        isOnline: true,
        sessionDuration: 8 * 60 * 1000
      },
      {
        customerId: 3,
        customerEmail: 'jan.de.vries@email.com',
        customerName: 'Jan de Vries',
        socketId: 'socket_3',
        startTime: new Date(Date.now() - 25 * 60 * 1000),
        currentPage: '/customer-portal',
        userAgent: 'Firefox 119.0 / Windows 11',
        screenResolution: '1366x768',
        activities: [
          { type: 'navigation', page: '/customer-portal', timestamp: new Date(Date.now() - 20 * 60 * 1000) },
          { type: 'hover', element: 'Order details', timestamp: new Date(Date.now() - 15 * 60 * 1000) },
          { type: 'click', element: 'Track order', timestamp: new Date(Date.now() - 10 * 60 * 1000) }
        ],
        isOnline: true,
  ...
  const updateLiveActivities = () => {
    const newActivity: CustomerActivity = {
      type: ['click', 'scroll', 'hover', 'navigation'][Math.floor(Math.random() * 4)],
      element: ['Order button', 'Profile link', 'Product image', 'Search box'][Math.floor(Math.random() * 4)],
      timestamp: new Date()
    };

    setLiveActivities(prev => [newActivity, ...prev.slice(0, 49)]);
  };
